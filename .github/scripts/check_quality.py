...
def main() -> None:
    ...
    checks = [
        ...
        ('Runtime Type Checking (beartype)', ['uv', 'run', 'python', '.github/scripts/check_validation.py', '90']),
        ('Contract Validation (deal)', ['uv', 'run', 'python', '.github/scripts/check_validation.py', '90']),
        ('Data Validation (pydantic)', ['uv', 'run', 'python', '-c', 'from src.models import *; print("✅ Pydantic models valid")']),
        ('Complexity Check', ['uv', 'run', 'python', '.github/scripts/check_complexity.py', '15']),
        ('Docstring Coverage (interrogate)', ['uv', 'run', 'interrogate', '-v', 'src/', '--fail-under=80']),
