...
def _analyze_comments(comments: list[Any], config: dict[str, Any]) -> str:
    ...
    logging.info(f"Comment analysis: Positive={positive}, Negative={negative}, Net={net_score}")
...
def _update_timing_interval(state: dict[str, Any], num_comments: int, config: dict[str, Any]) -> None:
    ...
    elif state.get("currentIntervalSeconds", config["timing"]["firstCheck"]) < config["timing"]["maxWait"]:
        state["currentIntervalSeconds"] = state.get("currentIntervalSeconds", config["timing"]["firstCheck"]) + config["timing"]["increaseBy"]
...
def main() -> None:
    ...
    if now < (last_check + timedelta(seconds=interval)):
        logging.info(f"Not time yet. Next check in {int(((last_check + timedelta(seconds=interval)) - now).total_seconds())}s.")
    ...
    if github_output := os.environ.get("GITHUB_OUTPUT"):
        with Path(github_output).open("a") as f:
            f.write(f"state_changed={str(updated).lower()}\n")
    logging.info(f"Pulse check complete. Next interval: {state.get('currentIntervalSeconds', config['timing']['firstCheck'])}s")
