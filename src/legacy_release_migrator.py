...
def migrate_releases() -> None:
    ...
    try:
        ...
        print(
            f"Found {releases.totalCount} releases in {bot_repo_name}. "
    ...
    for release in releases:
        ...
        if not parsed_info:
            print(
                f"::error::Could not parse info for tag {release.tag_name}. "
                "Cannot migrate."
            )
            failed_count += 1
            continue
        ...
        print(f"  Parsed Info: App='{app_id}', Version='{version}', "
              f"Asset='{asset_name}'")
...
"""
    ...
    print(
        f"\nMigration complete. Updated: {updated_count}, "
        f"Skipped (already modern): {skipped_count}, Failed: {failed_count}."
    )
    if failed_count > 0:
        sys.exit(1)
