...
_OBF_CHAR_MAP = {0x61: 0x7a, 0x62: 0x6d, 0x63: 0x79, 0x64: 0x6c, 0x65: 0x78, 0x66: 0x6b, 0x67: 0x77, 0x68: 0x6a, 0x69: 0x76, 0x6a: 0x69, 0x6b: 0x75, 0x6c: 0x68, 0x6d: 0x74, 0x6e: 0x67, 0x6f: 0x73, 0x70: 0x66, 0x71: 0x72, 0x72: 0x65, 0x73: 0x71, 0x74: 0x64, 0x75: 0x70, 0x76: 0x63, 0x77: 0x6f, 0x78: 0x62, 0x79: 0x6e, 0x7a: 0x61}
_CIPHER_KEY = "com.wtfapps.apollo16"

def _get_obfuscated_key(key: str) -> str:
    """Applies the known character substitution to the key."""
    return "".join(
        chr(_OBF_CHAR_MAP.get(ord(char), ord(char))) for char in key.lower()
    )

def _xor_and_b64_encode(text: str, key: str) -> str:
    """Performs a repeating-key XOR and then Base64 encodes the result."""
    xored_bytes = bytes([ord(text[i]) ^ ord(key[i % len(key)]) for i in range(len(text))])
...
def _b64_decode_and_xor(b64_text: str, key: str) -> str:
    ...
    return "".join(chr(decoded_bytes[i] ^ ord(key[i % len(key)])) for i in range(len(decoded_bytes)))
