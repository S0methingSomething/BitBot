...
def main() -> None:
    ...
    non_legacy_releases = [
        r for r in releases if "-v" not in r.get("tag_name", "")
    ]
    ...
    print(
        f"Latest non-legacy release is {latest_release['tag_name']}. "
        f"Checking {len(older_releases)} older non-legacy release(s)."
    )
    ...
    for release in older_releases:
        ...
        try:
            ...
            print(
                f"::warning::Failed to update release {release_id}. "
