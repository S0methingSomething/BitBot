...
def _update_release_title(
    release: dict[str, Any], repo: str, headers: dict[str, str]
) -> bool:
    ...
    try:
        update_response = requests.patch(
            update_url, headers=headers, json=payload, timeout=10
        )
        ...
        logging.warning(
            f"Failed to update release {release['id']}. Status: {update_response.status_code}, Body: {update_response.text}"
        )
        return False
...
def main() -> None:
    ...
    logging.info(
        f"Latest non-legacy release is {latest_release['tag_name']}. Checking {len(older_releases)} older non-legacy release(s)."
    )

    updated_count = sum(
        1
        for release in older_releases
        if _update_release_title(release, bot_repo, headers)
    )
