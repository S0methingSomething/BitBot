...
def decrypt(encrypted_content: str, obfuscated_key: str) -> dict[str, Any]:
    ...
    for line in encrypted_content.splitlines():
        ...
        dec_key = b64_decode_and_xor(enc_key, obfuscated_key)
        dec_val_b64 = b64_decode_and_xor(enc_val, obfuscated_key)
...
def main() -> None:
    ...
    try:
        encrypted_content = input_file.read_text(encoding="utf-8")
        ...
        output_file.write_text(re_encrypted_content, encoding="utf-8")
        logging.info(f"Successfully processed and saved patched file to: {output_file}")
