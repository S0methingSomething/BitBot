...
def main() -> None:
    ...
    new_releases = [
        r for r in all_source_releases if r["id"] not in processed_release_ids
    ]
    ...
    for release in new_releases:
        print(f"--- Processing: {release['tag_name']} ({release['id']}) ---")

        parsed_info = parse_release_notes(
            release.get("body", ""), release["tag_name"], release["name"], config
        )
        if not parsed_info:
        ...
        try:
            ...
            original_file = download_asset(
                source_repo, release["id"], asset_name
            )
            ...
            print(
                f"::error::Failed to process app {display_name} from "
                f"release {release['tag_name']}. Reason: {e}"
            )
