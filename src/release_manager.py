...
def parse_release_description(description: str, apps_config: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for line in description.splitlines():
        ...
        if parsed_line := _parse_line(line, app_id_map):
...
def create_bot_release(bot_repo: str, tag: str, title: str, notes: str, file_path: Path) -> None:
...
def process_app(app: Dict[str, Any], bot_repo: str, source_repo: str, release_id: int, release_tag: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        return {
            "display_name": app["display_name"], "version": app["version"],
...
def process_release(release: Dict[str, Any], config: Dict[str, Any], processed_ids: List[int]) -> Dict[str, Any]:
    ...
    for app in apps_in_release:
        if result := process_app(app, config["github"]["botRepo"], config["github"]["sourceRepo"], release["id"], release["tag_name"]):
    ...
    if processed_count > 0:
        ...
        logging.info(f"Successfully processed {processed_count} app(s) from source release {release['tag_name']}. State updated.")
    return processed_data

def main() -> None:
    config = load_config()
    processed_ids = load_release_state()
    new_releases = [r for r in get_source_releases(config["github"]["sourceRepo"]) if r["id"] not in processed_ids]
    ...
    if all_processed_data:
        ...
        DOWNLOAD_DIR.mkdir(exist_ok=True)
        (DOWNLOAD_DIR / "releases.json").write_text(json.dumps(all_processed_data, indent=2))
