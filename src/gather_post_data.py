...
def _parse_and_add_release(
    release: Any, aggregated_data: dict[str, Any], config: Any
) -> None:
    ...
    app_data = aggregated_data.setdefault(
        app_id, {"display_name": parsed_info["display_name"], "releases_by_version": {}}
    )
    ...
    if not download_url:
        logging.warning(
            f"No matching asset found for release {release.tag_name}. Skipping."
        )
        return
...
def _process_releases(releases: Any, config: Any) -> dict[str, Any]:
...
def _find_best_release(
    release_group: list[dict[str, Any]], app_id: str
) -> dict[str, Any]:
    ...
    return (
        best or sorted(release_group, key=lambda x: x["published_at"], reverse=True)[0]
    )
...
def _finalize_data(aggregated_data: dict[str, Any]) -> dict[str, Any]:
    ...
    for app_id, app_info in aggregated_data.items():
        clean_releases = [
            _find_best_release(group, app_id)
            for group in app_info["releases_by_version"].values()
        ]
...
def main() -> None:
    ...
    logging.info(
        f"Successfully generated clean, structured release data at {paths.RELEASES_JSON_FILE}"
    )
