...
def main():
    ...
    for release in bot_releases:
        ...
        if app_id not in aggregated_data:
            aggregated_data[app_id] = {"display_name": parsed_info['display_name'], "releases_by_version": {}}
        
        if version not in aggregated_data[app_id]['releases_by_version']:
            aggregated_data[app_id]['releases_by_version'][version] = []

        download_url = next((asset.browser_download_url for asset in release.get_assets() if asset.name == parsed_info.get('asset_name')), None)
        if not download_url:
    ...
    for app_id, app_info in aggregated_data.items():
        ...
        for version, release_group in app_info['releases_by_version'].items():
            if len(release_group) == 1:
                ...
                best_release = next((r for r in release_group if r['tag_name'].startswith(f"{app_id}-v")), None)
                if not best_release:
        ...
        if clean_releases:
            final_data[app_id] = {"display_name": app_info['display_name'], "latest_release": clean_releases[0], "previous_releases": clean_releases[1:]}
        else:
             final_data[app_id] = {"display_name": app_info['display_name'], "latest_release": None, "previous_releases": []}
